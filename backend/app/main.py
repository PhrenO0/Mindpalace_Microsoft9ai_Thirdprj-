...
def vision_label(payload: DetectRequest) -> dict:
    ...
    system = (
        "너는 실내 3D 장면 사진에서 '정중앙(빨간 십자선/조준점) 바로 그 위치'에 있는 사물만 인식하는 도우미다."
        " 화면에서 더 크거나 옆에 있는 가구가 아니라, 오직 중앙 조준점이 찍힌 그 사물의 종류를 한국어 한 단어(일반 명사)로 답한다."
        " 중앙이 바닥·벽·천장·빈 공간이거나 무엇을 가리키는지 불분명하면 반드시 '없음'이라고 답한다. 사람·텍스트·UI·번호구슬은 사물이 아니다."
    )
    user_text = (
        "이미지 정중앙(십자선)이 가리키는 사물의 종류만 한국어 한 단어로 답하라"
        " (예: 의자, 소파, 식탁, 책상, 침대, 책장, 수납장, 화분, 조명, 그림, 러그, 창문, TV, 냉장고, 꽃병, 프린터, 싱크대)."
        " 중앙이 바닥/벽/빈 공간이면 '없음'. 옆의 더 큰 가구를 대신 답하지 말 것."
        ' JSON으로만: {"label":"<종류 또는 없음>"}'
    )
    body: dict[str, Any] = {
        "messages": [
...
def usage_track(payload: UsageTrackRequest, user_id: str = Depends(require_login)) -> dict:
    """프론트가 사용량을 알린다. 갱신된 누적 토큰을 돌려줘 실시간 카운터에 바로 반영하게 한다.
