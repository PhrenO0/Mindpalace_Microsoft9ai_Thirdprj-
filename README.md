...
> 라이브 데모에서 직접 캡처한 **실제 화면**입니다. 각 토글을 열면 화면과 **그 화면에 담은 UI/UX 의도**가 나옵니다. (이미지 원본: [`docs/ui/`](docs/ui/))

### 🚪 입구 — 온보딩

<details>
<summary><b>🏠 홈 — "내 PDF가 이런 공간이 된다"를 첫 화면에서 보여준다</b></summary>

<br>

![홈](docs/ui/01-home.png)

**의도.** 설명 문장보다 **결과물(3D 궁전)을 먼저** 보여줘 "이게 뭐 하는 서비스인가"를 0초에 이해시킨다. 히어로 한 장에 제품의 약속(자료 → 걸어다니는 기억의 공간)이 담겨 있고, CTA를 둘로 갈라 **목적이 분명한 사람(내 PDF로 시작하기)** 과 **구경부터 하고 싶은 사람(3D 둘러보기)** 을 모두 받는다. 따뜻한 호박색 톤·한옥 일러스트로 "공부 도구"의 딱딱함을 덜었다.
...
<details>
<summary><b>🌏 3D 지도 (종로) — 실제 지형 위에 방들이 명소로 떠오른다</b></summary>

<br>

![3D 지도 종로](docs/ui/04-vworld_map.png)

**의도.** VWorld 국가 3D 지도(실제 위성 지형) 위에 학습 방을 **명소 마커**로 띄운다. 경복궁·창덕궁·덕수궁처럼 **위치 관계가 분명한 실제 랜드마크**에 방을 걸어, "장소의 순서"가 곧 "기억의 순서"가 되게 했다. 마커를 누르면 우측 패널에 방 정보가 뜨고 '입장'으로 방 내부로 들어간다. 우상단 내비·우하단 미니맵으로 공간 감각을 유지한다.

</details>

<details>
<summary><b>🏯 3D 지도 (경주) — 한 도시에 묶이지 않는 다지역 지원</b></summary>

<br>

![3D 지도 경주](docs/ui/05-vworld_gyeongju.png)

**의도.** 같은 엔진이 도시만 바꾸면 어디서든 동작함을 보여준다(불국사·첨성대·석굴암…). 사용자가 **자기에게 가장 익숙한 도시**를 무대로 골라야 기억 효과가 커지므로, 전국 랜드마크 좌표를 정밀화해 다지역을 지원한다.

</details>

<details>
<summary><b>🚶 1인칭 워크스루 — 방을 걸으며 가구마다 개념을 외운다</b></summary>

<br>

![워크스루](docs/ui/06-memory-walk.png)

**의도.** 제품의 심장. WASD로 방을 **1인칭으로 걷고**, 가구마다 붙은 **번호 핫스팟(①②③…)** 을 동선 순서대로 지나며 외운다. 바닥의 동선 선은 "어디서 시작해 어떻게 돌지"를 안내하고, 번호는 입장 시점의 부채꼴 시야 기준으로 매겨진다(장소법). 좌측 패널(돌아가기·지도·방 이동·라이트·내 서재에 저장)과 우하단 미니맵으로 길을 잃지 않게 했다.

</details>

<details>
<summary><b>🎨 방 디자인 스튜디오 — 같은 지식, 다른 공간으로 갈아끼우기</b></summary>

<br>

![방 디자인](docs/ui/07-glb-customizer.png)

**의도.** 같은 학습 내용을 **사용자가 선호하는 방 프리셋**(거실·주방·교실·사무실…)으로 바꿔 담을 수 있는 갤러리. 익숙하거나 기억에 잘 남는 공간일수록 암기가 잘 되기 때문에, "내 공간"을 고르는 선택지를 넓혔다.
